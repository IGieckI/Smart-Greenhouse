...
void lora_init() {
    ...
    // Use SPI3_HOST for the node to keep it distinct from typical default allocations
    ...
    // Match the Gateway: 868.0 MHz, 125 kHz, SF9, CR7, 0x12 sync word
...
// GET /dump -> Download the whole RingBuffer
...
// POST /set_time -> Synchronize system clock
...
// CoAP Server Handlers & Task

// GET /telemetry (Observe/Subscribe)
    ...
    while(1) {
        if (xQueueReceive(telemetry_queue, &incomingData, portMAX_DELAY)) {
            ...
            snprintf(lora_payload, sizeof(lora_payload), 
                     "{\"ts\":%lu,\"id\":%lu,\"p\":%.2f,\"wt\":%.2f,\"lux\":%.2f,\"tds\":%.2f,\"sm\":%.2f,\"at\":%.2f,\"h\":%.2f,\"lt\":%.2f}",
                     (unsigned long)incomingData.timestamp, (unsigned long)incomingData.node_id, 
                     incomingData.pressure, incomingData.water_temp, incomingData.light_lux, 
                     incomingData.tds_value, incomingData.soil_moisture, incomingData.air_temp, 
                     incomingData.humidity, incomingData.leaf_temp);
            ...
            // Update global CoAP variable and notify clients
            ...
            // Push to Ring Buffer for HTTP download
...
extern "C" void app_main(void)
{
    ...
    // Retrieve and print MAC for debugging/setup
    uint8_t mac[6];
