...
esp_err_t SoilMoistureSensor::init(const Config& config) {
    ...
    init_config.unit_id = config.adc_unit;
    ...
    chan_config.atten = ADC_ATTEN_DB_12; // 12dB attenuation allows reading up to ~3.1V

    ret = adc_oneshot_config_channel(_adc_handle, _adc_channel, &chan_config);
    if (ret != ESP_OK) {
