...
from utils import fetch_api, fetch_api_raw, build_keyboard, check_spam_lock, unzip_and_send
...
async def handle_training_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "train_menu":
        ...
        try:
            ...
            await unzip_and_send(update, wait_msg, zip_bytes, f"Global Analytics ({freq}m)")
        finally:
        ...
        try:
            ...
            await unzip_and_send(update, wait_msg, zip_bytes, f"Task Analytics: {task.upper()} ({freq}m)")
        finally:
            context.user_data['is_processing'] = False
