...
echo "Accessing Grafana container with user: $G_USER"
...
if [ "$HTTP_STATUS" -eq 401 ]; then
    echo "Grafana authentication failed (401 Unauthorized)."
    exit 1
...
if [ -z "$DASH_UIDS" ] || [ "$DASH_UIDS" == "null" ]; then
    echo "No dashboards found via the API."
    echo "Details: $HTTP_BODY"
    exit 0
fi
...
for DASH_ID in $DASH_UIDS; do
    ...
    echo "Dashboard successfully stored in exported_dashboards/${TITLE}.json"
done