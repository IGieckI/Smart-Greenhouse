...
def _clean_feature_name(name: str) -> str:
    """Cleans up extremely long feature names generated by Scikit-Learn pipelines."""
...
def _get_best_model_for_task(models_data: dict) -> str:
    """Finds the best model name based strictly on minimum MAE."""
    best_model = None
    min_mae = float('inf')
...
# ==========================================
# MAIN PLOT GENERATORS
# ==========================================
def generate_task_plots(task_dir: str, task_name: str) -> list:
    """Generates task-specific analytical plots and returns paths."""
...
def generate_global_plots(base_dir: str, freq_minutes: int) -> list:
    """Scans all tasks, extracts metrics, generates global comparative matrices."""
    ...
    generated_files.append(_generate_global_heatmap(all_data, model_names, "training_time_seconds", "performance", global_dir, freq_minutes, best_models_global, cmap="Reds"))
    generated_files.append(_generate_global_heatmap(all_data, model_names, "inference_time_seconds", "performance", global_dir, freq_minutes, best_models_global, cmap="Reds"))
...
# ==========================================
# INTERNAL PLOTTING UTILS (GLOBAL)
# ==========================================
def _generate_global_heatmap(all_data, model_names, key, category, out_dir, freq, best_models, cmap="YlGnBu"):
    ...
    # Red box ALWAYS highlights the model with the best MAE
...
def _generate_global_params_grid(all_data, model_names, out_dir, freq, best_models):
    ...
    for r_idx, task in enumerate(tasks):
        ...
        if best_m in model_names:
            ...
            cell = table[r_idx + 1, c_idx] # +1 because of header row
            cell.set_edgecolor('red')
            cell.set_linewidth(4)
    
    plt.title(f"Global Matrix: BEST HYPERPARAMETERS ({freq}m)\nRed box indicates best MAE performer", pad=20, fontsize=16)
    plt.tight_layout()
    plt.savefig(file_path)
    plt.close()
    return file_path
...
def _generate_global_feature_importance_grid(all_data, model_names, out_dir, freq, best_models):
    ...
    # Wider figure to accommodate legends
        ...
        for c_idx, model_name in enumerate(model_names):
            ...
            if model_name in all_data[task]:
                ...
                if top_feats:
                    ...
                    # Use legend instead of labels on pie slices
...
# ==========================================
# INTERNAL PLOTTING UTILS (TASK SPECIFIC)
# ==========================================
def _plot_metrics_comparison(models_data, model_names, task_name, output_dir, best_model):
    ...
    # Highlight label if it's the best model
