...
from shared_core.preprocessing import build_advanced_features, get_extended_features_list
...
def log_and_evaluate(y_test, y_pred, features_names, model, model_name, training_time, inf_time, best_params, archive_dir, plots_dir):
    ...
    # Se c'è il ColumnTransformer, estraiamo i nomi veri per mappare i coefficienti!
...
def run_pipeline_for_task(task_name, config, df_clean):
    ...
    # 1. Otteniamo la lista delle feature estese a monte
    extended_features_list = get_extended_features_list(features_list, use_lags)

    df_train_final_list, df_test_final_list = [], []

    # 2. Elaborazione per Singola Board (Cruciale per derivate e lag)
    for board_id in ACTIVE_BOARDS:
        df_b = df_clean[df_clean['id_board'] == board_id].copy()
        if df_b.empty: continue
        
        # A. Split Cronologico per singola board
        split_idx = int(len(df_b) * TRAIN_SPLIT_PERCENTAGE)
        df_train_b = df_b.iloc[:split_idx]
        df_test_b = df_b.iloc[split_idx:]

        # B. Feature Engineering (Tempo + Derivate) - DEVE avvenire prima dei lag!
        df_train_b = build_advanced_features(df_train_b, features_list, use_lags)
        df_test_b = build_advanced_features(df_test_b, features_list, use_lags)

        # C. Creazione Lag ISOLATA (Nessuna contaminazione tra board)
        if use_lags:
            df_train_b = create_lagged_features(df_train_b, target_col, extended_features_list, lags=DEFAULT_LAGS, lag_target=lag_target)
            df_test_b = create_lagged_features(df_test_b, target_col, extended_features_list, lags=DEFAULT_LAGS, lag_target=lag_target)
    ...
    # 4. Selezione delle colonne e pulizia finale
    if use_lags:
    ...
    # Taglio dei buchi temporali (drop NaN generati da diff() e shift())
    ...
    # Costruiamo un trasformatore selettivo: Polinomi di grado 3 SOLO per le feature originali di base
    # (Per non far esplodere la dimensionalità con lag e derivate)
    poly_transformer = ColumnTransformer(
        transformers=[
            ('poly', PolynomialFeatures(degree=3, include_bias=False), features_list)
        ],
...
#     print(f"[{task_name}] Estrazione feature cicliche e derivate...")
#     df_train_raw = build_advanced_features(df_train_raw, features_list, use_lags)
#     df_test_raw = build_advanced_features(df_test_raw, features_list, use_lags)
    
#     # Otteniamo la nuova lista delle feature estese (base + time + diff)
#     extended_features_list = get_extended_features_list(features_list, use_lags)
...
#     if use_lags:
#         print(f"[{task_name}] Creazione feature ritardate (lags={DEFAULT_LAGS})...")
#         df_train_final = create_lagged_features(df_train_raw, target_col, extended_features_list, lags=DEFAULT_LAGS, lag_target=lag_target)
#         df_test_final = create_lagged_features(df_test_raw, target_col, extended_features_list, lags=DEFAULT_LAGS, lag_target=lag_target)
        
#         # Le feature per il modello sono le estese + i loro lag
#         model_features = [col for col in df_train_final.columns if ('lag' in col and (lag_target or target_col not in col)) or col in extended_features_list]
#     else:
#         print(f"[{task_name}] Training puntuale (senza lag)...")
#         df_train_final = df_train_raw.copy()
...
#     # Costruiamo un trasformatore selettivo: Polinomi di grado 2 SOLO per le feature di base originali
#     # Il resto (lag, time, derivate) passa inalterato ('passthrough')
#     poly_transformer = ColumnTransformer(
#         transformers=[
#             ('poly', PolynomialFeatures(degree=3, include_bias=False), features_list)
#         ],
...
#         "Ridge": {
#             "model": Pipeline([
#                 ('poly_features', poly_transformer), # Polinomio parziale!
#                 ('scaler', MinMaxScaler()), 
#                 ('regressor', Ridge())
#             ]),
#             "params": {
